...
const TracksList: React.FC<TrecksListProps> = ({
	role,
	nameSpec,
	currentUserId,
}) => {
	...
	useEffect(() => {
		if (data?.esTrackList?.items) {
			const latestYear = data.esTrackList.items
				.map((track: any) => track.data.recruitmentYear)
	...
	const groupedTracks = useMemo(() => {
		...
		if (data?.esTrackList?.items) {
			const grouped: any = data.esTrackList.items.reduce(
				(acc: { [key: string]: any[] }, track: any) => {
